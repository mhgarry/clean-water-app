...
const SideNav: React.FC<SideNavProps> = ({ categories = [] }) => {
  return (
    <aside>
      <div className="bg-gradient-to-tr from-accent via-card to-background text-card-foreground  flex-col md:px-2 lg:px-4 p-4 space-y-4 min-h-screen border  shadow h-screen sticky rounded-md hidden md:flex">
        {categories.map((category: NavPage) => (
          <div key={category.title}>
            <h2 className="md:text-sm xl:text-base font-bold text-foreground mb-2 ml-4">
              {category.title}
            </h2>
              ...
              {category.pages.map((page: NavPage) => (
                <li key={page.path}>
                  <Link
                    ...
                    hover:font-bold
                    hover:text-background hover:bg-foreground md:px-2 px-4 py-2 rounded-md flex flex-row gap-2
